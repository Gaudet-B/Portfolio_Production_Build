...
const Checkout = props => {
    ...
    // props 
    const { setPage, shoppingCart, getLocalOrDefault, detailShow, setDetailShow, stepDetail } = props

    const [orderDetails, setOrderDetails] = useState(shoppingCart)
    ...
    const handleNext = () => setDetailStep(detailStep + 1)

    const handleBack = () => setDetailStep(detailStep - 1)

    const handleNew = () => setPage("shop")
        ...
        <div className={styles.shop} >
            <Container className="d-flex flex-column my-3" style={{ width: "360px", padding: "0px" }}>
                <div className="bg-light rounded text-secondary d-flex flex-column text-start my-2 p-3" style={{ width: "auto" }}>
                    ...
                    {(detailStep < 0  || detailStep > orderDetails.length) ?
                    handleCloseDetails()
                    :
                    <Container style={{ backgroundColor: "rgb(143, 3, 3)", padding: "0px" }}>
                        <Modal.Header className="bg-light" closeButton>
                            <div className="fs-4 fw-bold mb-0" style={{ color: "rgb(143, 3, 3)" }} ><p className={styles.cartHeader}>PIZZA #{detailStep + 1}</p></div>
                            <img
                                src={pizzaCart}
                                ...
                                className="mx-4"
                            />
                            <div className="fs-4 fw-bold mb-0 ms-1" style={{ color: "rgb(143, 3, 3)" }} ><p className={styles.cartHeader}>${orderDetails[detailStep].price}</p></div>
                        </Modal.Header>
                        <Modal.Body className="text-light">
                        {(!orderDetails[detailStep]) ?
                        null
                        :
                        <div className="d-flex flex-column text-center border border-light rounded p-3" >
                            <div className="d-flex flex-row">
                                <img
                                    src={getCrustImg(orderDetails[detailStep])}
                                    alt=""
                                    height="70px"
                                    width="70px"
                                    className="m-1 border border-light rounded"
                                />
                                {(!orderDetails[detailStep].crust) ? null
                                :
                                <div className="d-flex flex-row">
                                    <p className="fs-2 ms-3 mb-0 mt-3 text-wrap" >{orderDetails[detailStep].crust.name} crust</p>
                                </div>
                                }
                            </div>
                            <div className="d-flex flex-row my-3">
                                <img
                                    src={getSauceImg(orderDetails[detailStep])}
                                    alt=""
                                    height="70px"
                                    width="70px"
                                    className="m-1 border border-light rounded"
                                />
                                {(!orderDetails[detailStep].sauce) ? null
                                :
                                <p className="fs-2 ms-3 mb-0 mt-3 text-wrap" >{orderDetails[detailStep].sauce.name}</p>
                                }
                            </div>
                            {(!orderDetails[detailStep].toppings) ? null
                            :
                            <div>
                            <div className="d-flex flex-row">
                                <p className="mt-0 mb-2 me-4 fs-3" >Cheese:</p>
                                <p className="fs-6 ms-4 mb-0 mt-2">
                                {orderDetails[detailStep].toppings.cheese.map((cheese, idx) => {
                                    if (idx == orderDetails[detailStep].toppings.cheese.length - 1) return(
                            ...
                            <p className="mt-0 mb-2 me-4 fs-3" >Meat:</p>
                                ...
                                {orderDetails[detailStep].toppings.meat.map((meat, idx) => {
                                    if (idx == orderDetails[detailStep].toppings.meat.length - 1) return(
                            ...
                            <p className="mt-0 mb-2 fs-4" >Toppings:</p>
                                <p className="fs-6 ms-5 mb-0">
                                {orderDetails[detailStep].toppings.other.map((other, idx) => {
                                    if (idx == orderDetails[detailStep].toppings.other.length - 1) return(
                    ...
                    <Modal.Footer className="d-flex flex-row justify-content-between">
                        ...
                        <div>
                            {(detailStep >= orderDetails.length - 1) ? null
                            :
